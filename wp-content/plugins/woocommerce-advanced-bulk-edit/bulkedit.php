...
//				_e( 'Sale Price', 'wooadvbulkedit');
			...
			>
				...
				<template id="btn-info-filter-regular-price-body">
					<?php _e('The "filter expression" option enables filtering using an advanced expression that allows specifying a mix of individual prices as well as range of prices. For example:<br /><br />', 'woocommerce-advbulkedit'); ?>
					<?php _e('10,20,45-57,80,90-100<br /><br />', 'woocommerce-advbulkedit'); ?>
					<?php _e('This will filter prices 10, 20 and 80 and also the range of prices between 45 and 57 and the ranges of prices between 90 and 100. The range includes the limiting values, e.g. 45 and 57.', 'woocommerce-advbulkedit'); ?>
				</template>
			</td>
...
<!--				 <input id="settings" class="button-primary-wcabe" type="button" value="--><?php //_e( "Show/Hide Fields", "woocommerce-advbulkedit"); ?><!--" />-->
...
<!--                    </tr>-->
					<tr>
						<td width="50%" style="padding-top: 20px;">
							<label><input id="setting-disable-hints" type="checkbox"
									...
									?>
								><?php _e( 'Disable hints (needs page reload)', 'woocommerce-advbulkedit'); ?></label>
						</td>
						<td  style="padding-top: 20px;">
							/<?php _e( 'Use it if you have issues with JS errors, caused by other plugins', 'woocommerce-advbulkedit'); ?>/
						</td>
					</tr>
					...
					<tr>
						<td width="50%" style="padding-top: 20px;">
							<label><input id="setting-enable-admin-only-visible" type="checkbox"
									...
									?>
								><?php _e( 'Only users with Administrator role can see this plugin', 'woocommerce-advbulkedit'); ?></label>
						</td>
						<td  style="padding-top: 20px;">
							/<?php _e( 'Use it to restrict the plugin access', 'woocommerce-advbulkedit'); ?>/
						</td>
					</tr>
					<?php }; ?>
				...
				<div id="pluginsettingstab-2">
					...
					$fixed_filter_options = [
						'showattributes' => __( 'Attributes', 'woocommerce-advbulkedit'),
						'showprices' => __( 'Regular/Sale Price', 'woocommerce-advbulkedit'),
						'showskutags' => __( 'SKU/Tags', 'woocommerce-advbulkedit'),
						'showdescriptions' => __( 'Long/Short Descriptions', 'woocommerce-advbulkedit'),
						'showidsearch' => __( 'ID', 'woocommerce-advbulkedit'),
						'showstocksearch' => __( 'Stock Qty', 'woocommerce-advbulkedit'),
					];
				...
				<tr data-id="_regular_price">
					...
					<td>
						 <select id="bulk_regular_price_round" style="display: none;">
							...
							<option value="roundup01"><?php _e( "round-up (0.01)", "woocommerce-advbulkedit"); ?></option>
							<option value="rounddown01"><?php _e( "round-down (0.01)", "woocommerce-advbulkedit"); ?></option>
				...
				if(is_array($this->attributes) && !empty($this->attributes) && $skiploadfrontpage)
				{
					...
					foreach($this->attributes as $attr)
					{
						...
						'<select id="bulkadd'.$key.'" class="bulkselect">'.
							...
							'<option value="remove">'.__('remove','woocommerce-advbulkedit').'</option></select><button class="butnewattribute button-wcabe button-be-wcabe" type="button"><span class="icon-plus-outline"></span>new</button><div class="divnewattribute">'
