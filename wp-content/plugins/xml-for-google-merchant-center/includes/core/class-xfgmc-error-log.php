 ...
 *
 *    `XFGMC_Error_Log::record( sprintf( '%1$s line: %2$s', 'Text', __LINE__ ) );`
 *
 * The legacy approach using the constructor is still supported for backward compatibility:
 *
 *    `new XFGMC_Error_Log( sprintf( '%1$s line: %2$s', 'Text', __LINE__ ) );`
 *
...
final class XFGMC_Error_Log {
	 ...
	 * Writes plugin logs.
	 * 
	 * Example: `XFGMC_Error_Log::record( 'Текст' );`.
	 * 
	...
	public function __construct( $text_to_log, $log_dir_name = XFGMC_PLUGIN_UPLOADS_DIR_PATH ) {
		...
		_deprecated_function( 'XFGMC_Error_Log::__construct', '4.1.0', 'XFGMC_Error_Log::record()' );
	 ...
	 * @return void
	 */
	public static function record( $text_to_log, $log_dir_name = XFGMC_PLUGIN_UPLOADS_DIR_PATH ) {

		if ( ! wp_mkdir_p( $log_dir_name ) ) {
			error_log( sprintf( 'XFGMC: XFGMC_Error_Log: %1$s: %2$s. %3$s.',
				'There is no folder',
				$log_dir_name,
		...
		// /home/site.ru/public_html/wp-content/uploads/xfgmc/xml-for-google-merchant-center.log
		$log_file_path = trailingslashit( $log_dir_name ) . 'xml-for-google-merchant-center.log';
		...
		if ( false === $result ) {
			error_log( "XFGMC: XFGMC_Error_Log: Failed to write to log file: $log_file_path" );
		}
	...
	protected static function keeplogs_status() {
		...
		if ( is_multisite() ) {
			$v = get_blog_option( get_current_blog_id(), 'xfgmc_keeplogs', 'disabled' );
		} else {
			$v = get_option( 'xfgmc_keeplogs', 'disabled' );
		}
